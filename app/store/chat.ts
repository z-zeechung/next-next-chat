...
export interface ChatSession {
  id: string;
  topic: string;
  emoji?: string; // 对话图标，html形式
  avatar?: string // assistant的头像，html形式
  prompt?: string // 角色提示词
  greeting?: string
  live2d?: {
    src: string
...
function countMessages(msgs: Message[]) {
  return msgs.reduce(
    (pre, cur) => pre + estimateTokenLength(/*getMessageTextContent(cur)*/cur.content),
    0,
  );
}
  ...
  (set, _get) => {
    ...
    const methods = {
      ...
      deleteSession(index: number, silent=false) {
        ...
        const deletedMessages = JSON.parse(JSON.stringify(deletedSession?.messages))
      ...
      /** @deprecated */
      onUserInput(content: string, attachImages?: string[], prompt="", smart=false, tools=[]) {
        return new ControllablePromise((resolve, reject, abort)=>{
          ...
          let userMessage: Message = {type:"text", role:"user", content:mContent}

          const botMessage: Message = {type:"text", role:"assistant", content:"", streaming:true}

          // get recent messages
          const recentMessages = get().getMessagesWithMemory(prompt=prompt);
          ...
          const chatPromise = ClientApi.chat(
            ...
            {
              model:smart?"smart":"regular",
              tools:tools
            }
          )
          chatPromise.then(message=>{
              botMessage.streaming = false;
              if (message) {
                botMessage.content = message;
                get().onNewMessage(botMessage);
      ...
      getMemoryPrompt() {
        ...
        if (session.memoryPrompt.length) {
          return {type:"text", role:"system", content:Locale.Store.Prompt.History(session.memoryPrompt)} as Message
          // {
          //   role: "system",
      ...
      getMessagesWithMemory(prompt="") {
        ...
        const reversedRecentMessages = [] as Message[];
        for (
          ...
          tokenCount += estimateTokenLength(/*getMessageTextContent(msg)*/msg.content);
      ...
      summarizeSession() {
        ...
        if (
          ...
          const topicMessages = messages.concat(
            {type:"text", role:"user", content:Locale.Store.Prompt.Topic}
          );
          // api.llm.chat({
        ...
        if (
          ...
          //     new MarkdownMessage("system", Locale.Store.Prompt.Summarize)
          //   ),
          //   config: {
          //     ...modelcfg,
          //     stream: true,
          ...
          ClientApi.chat(
            toBeSummarizedMsgs.concat(
              {type:"text", role:"system", content:Locale.Store.Prompt.Summarize}
            ),
            message=>{
              session.memoryPrompt = message;
            }
