...
class Node {
  ...
  constructor(key: string, value: number[], document: string, content:string) {
    this.key = key;
    this.value = value;
...
export class HNSW {
  ...
  loadIndex(mememoIndex/*: MememoIndexJSON*/) {
    ...
    this.graphLayers = mememoIndex.graphLayers;
    (this.nodes as any).graphLayers = mememoIndex.graphLayers
  }
   ...
   */
  async insert(key: string, value: number[], document: string, content:string, maxLevel?: number | undefined, ) {
    ...
    if (await this.nodes.has(key)) {
      ...
      if (nodeInfo.isDeleted) {
        ...
        await this.update(key, value, document, content);
        return;
      }
    ...
    await this.nodes.set(key, new Node(key, value, document, content));
   ...
   * this value in most cases. We add this parameter for testing purpose.
   */
  async bulkInsert(keys: string[], values: number[][], document: string, content:string[], maxLevels?: number[]) {
    ...
    const newNodes: Node[] = [];
    for (const [i, key] of keys.entries()) {
      newNodes.push(new Node(key, values[i], document, content[i]));
    }
   ...
   * @param value The embedding of the new element to insert.
   */
  async insertSkipIndex(key: string, value: number[], document: string, content:string) {
    // If the key already exists, throw an error
    if (await this.nodes.has(key)) {
   ...
   * @param values The embeddings of the new elements to insert.
   */
  async bulkInsertSkipIndex(keys: string[], values: number[][], document: string, content:string[]) {
    // If the key already exists, throw an error
    const existingKeys = await this.nodes.keys();
   ...
   */
  async update(key: string, value: number[], document: string, content:string) {
    if (!(await this.nodes.has(key))) {
      throw Error(
  ...
  async query(
    ...
    return {
      // keys,
      contents: keys.map(key=>this.nodes.nodesMap.get(key)?.content??""),
      distances
    };
  }
